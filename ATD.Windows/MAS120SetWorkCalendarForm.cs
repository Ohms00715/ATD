...
{
    public partial class MAS120SetWorkCalendarForm : Form
    {
        ...
        private DataSet _dsDaily = null ;
        #endregion variable declare

        public MAS120SetWorkCalendarForm(string connStr, string userID)
        {
            ...
            #region Daily
            this._dsDaily = this._sqlManager.GetRecordLDailyByMonthYear(System.DateTime.Today.Month.ToString(), System.DateTime.Today.Year.ToString());
            this.SetDataGridDaily();
            this.SetObjectDaily();
        ...
        private void INC130SetWorkCalendarForm_Load(object sender, EventArgs e)
        {
            #region default year
            this.txtYear.Text = System.DateTime.Now.Year.ToString();
            this.setDefaultCurrentRow();
            #endregion 
        }
        ...
        private void setDefaultCurrentRow()
        {
            this.getDataByYear();
            ...
            if (this.dgvMonth.ColumnCount >= iMonth)
            {
                indexRow = iMonth - 1;
                this.dgvMonth.Rows[indexRow].Selected = true;              
                int yyyy, mm;
                int.TryParse(this.dgvMonth.Rows[indexRow].Cells["COL_YYYY"].Value.ToString(), out yyyy);
                int.TryParse(this.dgvMonth.Rows[indexRow].Cells["COL_MM"].Value.ToString(), out mm);
                this.SetMonth(yyyy, mm);

                this.txtMonth.Text = this.dgvMonth.Rows[indexRow].Cells["COL_MM"].Value.ToString();
                this.txtMonthName.Text = this.dgvMonth.Rows[indexRow].Cells["COL_MMNAME"].Value.ToString();
                this.stxtYear.Text = this.dgvMonth.Rows[indexRow].Cells["COL_YYYY"].Value.ToString();
                this.GetDailyByMonth(mm.ToString(), yyyy.ToString());
            }
        }
        private void dgvMonth_DoubleClick(object sender, EventArgs e)
      {
        ...
        #region Daily
        #region support 
        private void SetDataGridDaily()
        {
            DataGridViewTextBoxColumn colTxt = null;
            DataGridViewCheckBoxColumn colChk = null;
            DataGridViewButtonColumn colBt = null;

            this.dgvTeam.AutoGenerateColumns = true;
            //this.dgvTeam.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;


            //colTxt = null;
            //colTxt = new DataGridViewTextBoxColumn();
            //colTxt.Name = "WrkDay";
            //colTxt.DataPropertyName = "WrkDay";
            //colTxt.HeaderText = "Daily";
            //colTxt.Width = 100;
            //colTxt.ReadOnly = true;
            //colTxt.Frozen = true;
            //colTxt.DefaultCellStyle.Format = "dd/MM/yyyy (ddd)";
            ...
            ////colTxt.Name = "WrkDay";
            ////colTxt.DataPropertyName = "WrkDay";
            ////colTxt.HeaderText = "Date";
            ////colTxt.Width = 50;
            ////colTxt.ReadOnly = true;
            ////colTxt.Frozen = true;
            ////colTxt.DefaultCellStyle.Format = "ddd";
            ////colTxt.SortMode = DataGridViewColumnSortMode.NotSortable;
            ////this.dgvDaily.Columns.Add(colTxt);

            //colTxt = null;
            //colTxt = new DataGridViewTextBoxColumn();
            //colTxt.Name = "PeriodInMonth";
            //colTxt.DataPropertyName = "PeriodInMonth";
            //colTxt.HeaderText = "Period In Month";
            //colTxt.Width = 100;
            //colTxt.ReadOnly = false;
            //colTxt.Frozen = true;
            //colTxt.MaxInputLength = 1;
            //colTxt.SortMode = DataGridViewColumnSortMode.NotSortable;
            //colTxt.DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomRight;
            //this.dgvTeam.Columns.Add(colTxt); 

            //colTxt = null;
            //colTxt = new DataGridViewTextBoxColumn();
            //colTxt.Name = "PlanQty";
            //colTxt.DataPropertyName = "PlanQty";
            //colTxt.HeaderText = "CSP Plan Qty";
            //colTxt.ToolTipText = "CSP Total Plan"; 
            //colTxt.Width = 100;
            //colTxt.ReadOnly = false;
            //colTxt.Frozen = true;
            //colTxt.DefaultCellStyle.Format = "##,##0.000";
            //colTxt.SortMode = DataGridViewColumnSortMode.NotSortable;
            //colTxt.DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomRight;
            //this.dgvTeam.Columns.Add(colTxt);
            ...
            //colTxt = null;
            //colTxt = new DataGridViewTextBoxColumn();
            //colTxt.Name = "FMPlanQty";
            //colTxt.DataPropertyName = "FMPlanQty";
            //colTxt.HeaderText = "FM Plan Qty";
            //colTxt.ToolTipText = "FM Total Plan"; 
            //colTxt.Width = 100;
            //colTxt.ReadOnly = false;
            //colTxt.Frozen = true;
            //colTxt.DefaultCellStyle.Format = "##,##0.000";
        ...
        private void SetObjectDaily()
        {
            ...
            //Binding cb;
            //cb = null;
            //cb = new System.Windows.Forms.Binding("Text", this.bdsDaily, "TotalMonthQty", true);
            //cb.FormatString = "#,##0.000";
            //this.txtPlanQty.DataBindings.Add(cb);

            //cb = null;
            //cb = new System.Windows.Forms.Binding("Text", this.bdsDaily, "TotalMonthQtyFM", true);
            //cb.FormatString = "#,##0.000";
            //this.txtFMPlanQty.DataBindings.Add(cb);

        }
        private void ClearDaily()
        {
            foreach (DataTable dt in this._dsDaily.Tables)
            {
                dt.Rows.Clear();
            }
        }
        private void GetDailyByMonth(string paMonth, string paYear)
        {
            this.ClearDaily();
            this._dsDaily.Merge(this._sqlManager.GetRecordLDailyByMonthYear(paMonth, paYear));
        }
        #endregion 
        #region update 
        private bool validateSaveDaily()
        {
            try
            {
                StringBuilder sb = new StringBuilder() ;
                ...
                {
                    lbConv = decimal.TryParse(dgv.Cells["planQty"].Value.ToString(), out ldcTemp);
                    dtWrkDate = (DateTime)dgv.Cells["WrkDay"].Value;

                    if (ldcTemp < 0)
                    {                    
                        sb.AppendLine(string.Format(" Daily {0} ยอดแผนต้องมากกว่าหรือเท่ากับ 0 .. CSP Plan Qty {1}" ,
                            dtWrkDate.ToString("dd/MM/yyyy") ,  ldcTemp.ToString("#,##0.000")));
                    }

                    lbConv = decimal.TryParse(dgv.Cells["FMplanQty"].Value.ToString(), out ldcTemp);
                    if (ldcTemp < 0)
                    {                        
                        sb.AppendLine(string.Format(" Daily {0} ยอดแผนต้องมากกว่าหรือเท่ากับ 0 .. FM Plan Qty {1}",
                            dtWrkDate.ToString("dd/MM/yyyy"), ldcTemp.ToString("#,##0.000")));
                    }
                ...
                if( sb.Length > 0 ) 
                {
                      MessageBox.Show("   พบยอดข้อมูลแผนน้อย 0  ดังนี้  \n\r" + sb.ToString() ,                                       
                                      "Plan Quantity Error",
                                      MessageBoxButtons.OK,
                                      MessageBoxIcon.Error);
                return false;
            ...
            {
                MessageBox.Show("  Call Method :: validateSaveDaily()" +
                                       "\n\r Message:" + exp.Message.ToString() +
                                       "\n\r Source: " + exp.Source.ToString() +
                                       "\n\r StackTrace: " + exp.StackTrace.ToString(),
                                      "Error",
        ...
        private void SaveDailyPlanToDB()
        {
            if (!this.validateSaveDaily())
            {
                return;
            }

            if (this._sqlManager.SaveDailyPlanToDB(ref this._dsDaily) < 0)
            {
                MessageBox.Show(" บันทึกข้อมูลไม่สำเร็จ \n\r" +this._sqlManager.LastError ,                                       
                                      "Update Fail..",
                                      MessageBoxButtons.OK,
                                      MessageBoxIcon.Error);
            }
        ...
        private void tsbSaveDaily_Click(object sender, EventArgs e)
        {
            this.Hourglass(true);
            this.SaveDailyPlanToDB();
            this.Hourglass(false);
        }
        #endregion 
        ...
        private void dgvCale_KeyUp(object sender, KeyEventArgs e)
        {
            if ((e.Shift && e.KeyCode == Keys.Insert) || (e.Control && e.KeyCode == Keys.V))
            {
                ...
                int r = dgvCale.SelectedCells[0].RowIndex;

                int c = dgvCale.SelectedCells[1].ColumnIndex;

                //add rows into grid to fit clipboard lines

                if (dgvCale.Rows.Count < (r + rowsInClipboard.Length))
                {

                    dgvCale.Rows.Add(r + rowsInClipboard.Length - dgvCale.Rows.Count );
                ...
                for (int iRow = 0; iRow < rowsInClipboard.Length; iRow++)
                {
                    ...
                    for (int iCol = 0; iCol < valuesInRow.Length; iCol++)
                    {
                        ...
                        //assign cell value, only if it within columns of the grid
                            ...
                            dgvCale.Rows[r + iRow].Cells[c + iCol].Value = valuesInRow[iCol];
                            continue;
                        }
